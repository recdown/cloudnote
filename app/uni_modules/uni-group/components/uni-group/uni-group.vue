<template>
  <view :class="['uni-group--'+mode ,margin?'group-margin':'']" :style="{marginTop: `${top}px` }" class="uni-group">
    <slot name="title">
      <view v-if="title" :style="{'padding-left':border?'30px':'15px'}" class="uni-group__title">
        <text class="uni-group__title-text">{{ title }}</text>
      </view>
    </slot>
    <view :class="{'group-conent-padding':border}" class="uni-group__content">
      <slot/>
    </view>
  </view>
</template>
